...
//Connection Settings @0-765244B9
$CCConnectionSettings = array (
    "Datos" => array(
        ...
        "BooleanFormat" => array("V", "F", ""),
        "Uppercase" => false
    )
);
//End Connection Settings

include(RelativePath . "/configure.php");
include(RelativePath . "/funciones.php");

//Initialize Common Variables @0-77613839
...
$CCSLocales->AddLocale("es", Array("es", "ES", array("V", "F", ""), 2, ",", ".", array("enero", "febrero", "marzo", "abril", "mayo", "junio", "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"), array("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), array("domingo", "lunes", "martes", "miércoles", "jueves", "viernes", "sábado"), array("dom", "lun", "mar", "mié", "jue", "vie", "sáb"), array("D", "L", "M", "M", "J", "V", "S"), array("dd", "/", "mm", "/", "yyyy"), array("dddd", ", ", "dd", " de ", "mmmm", " de ", "yyyy"), array("H", ":", "nn"), array("H", ":", "nn", ":", "ss"), "", "", 1, false, "", "windows-1252", "Windows-1252"));
...
//Datos Connection Class @-BA816E52
class clsDBDatos extends DB_MySQL
{
    ...
    function Initialize()
    {
        ...
        $Configuration = $CCConnectionSettings["Datos"];
    ...
    {
        if (method_exists($this, "has_next_row")) 
            return $this->has_next_row();
        if (count($this->NextRecord)) 
        return true;
        $Record = $this->Record;
        $result = parent::next_record();
        if ($result)
    ...
    function OptimizeSQL($SQL)
    {
        ...
        if (strcmp($this->RecordsCount, "CCS not counted")) 
            $SQL .= $this->Type == "MySQL" ? (" LIMIT " . (($Page - 1) * $PageSize) . "," . $PageSize): (" LIMIT " . $PageSize . " OFFSET " . (($Page - 1) * $PageSize));
        else
            $SQL .= $this->Type == "MySQL" ? (" LIMIT " . (($Page - 1) * $PageSize) . "," . ($PageSize + 1)): (" LIMIT " . ($PageSize + 1) . " OFFSET " . (($Page - 1) * $PageSize));
        return $SQL;
    }
...
function CCStrLen($str, $encoding = false) {
    ...
    return $encoding ? mb_strlen($str, $encoding) : strlen($str);
}
//End CCStrLen
...
function CCSubStr($str, $offset, $length = null, $encoding = false) {
    global $FileEncoding;
    if (false === $encoding) $encoding = $FileEncoding;
    return $encoding ? @mb_substr($str, $offset, $length, $encoding) : substr($str, $offset, $length);
}
//End CCSubStr
...
function CCStrPos($haystack, $needle, $offset = "", $encoding = false) {
    global $FileEncoding;
    if (false === $encoding) $encoding = $FileEncoding;
    return $encoding  ? mb_strpos($haystack, $needle, $offset, $encoding) : strpos($haystack, $needle, $offset);
}
//End CCStrPos
...
//CCLoginUser @0-6D1A9BCE
function CCLoginUser($login, $password)
{
    $db = new clsDBDatos();
    $SQL = "SELECT usuario_id, nivel_acceso_id FROM usuarios WHERE log_usuario=" . $db->ToSQL($login, ccsText) . " AND clave=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if($Result)
    {
        CCSetSession("UserID", $db->f("usuario_id"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("nivel_acceso_id"));
    }
    $db->close();
    return $Result;
}
